...
const courses = [
  ...
  },
  {
    ...
    desc: "Master advanced Python concepts such as OOP, modules, APIs, data processing, automation.",
...
const Header = () => {
  ...
  const {user} =useUser();
  return (
    <div className="p-4 max-w-7xl justify-between flex w-full">
      <div className="flex gap-2 items-center">
        <Image src={"/logo.png"} alt="logo" width={40} height={40} />
        <h2 className="font-game font-bold text-4xl">CodeBox</h2>
      </div>
              ...
              <ul className="grid grid-cols-2 gap-2 sm:w-[400px] md:w-[500px] lg:w-[600px]">
                {courses.map((course, index) => (
                  <div key={index} className="p-2 hover:bg-accent rounded-xl cursor-pointer">
                    <h2>{course.name}</h2>
                    <p className="text-sm text">{course.desc}</p>
                  </div>
                ))}
              </ul>
            </NavigationMenuContent>
     ...
     <Button className="font-game text-2xl" variant={"pixel"}>
        ...
        <Button className="font-game text-2xl" >Dashboard</Button></div>}
        <UserButton/>
    </div>
  );
};
